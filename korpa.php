...
<table class="table text_table">
    <tr><th colspan="6"><h3>Detalji narudžbe</h3></th></tr>
    <tr>
    ...
    foreach($_SESSION['shopping_cart'] as $key => $product):
        ...
        <td><?php echo number_format((float)$product['quantity'] * (float)$product['price'], 2); ?>&nbsp;KM</td>
        <td>
            ...
            <div class="col-sm-6 col-md-3 mb-4 text-center">
                <form method="post" action="korpa.php?action=add&id=<?php echo $product['id']; ?>">
                    <div class="textcard bojatekstakartice font-italic color p-2">
                        <img class="img-responsive img-fluid mx-auto d-block rounded-pill shadow p-1" id="velicina_slika_user" src="<?php echo $product['picture']; ?>" />
                        <h4 class="text-center"><?php echo $product['title'];?></h4>
                        <h5 class="text-center pt-3"><?php echo $product['price']; ?>&nbsp;KM</h5>
                        <input type="text" name="quantity" class="form-control" value="1"/>
                        <textarea type="text" name="details_of_order" class="form-control" placeholder="unesite i količinu i ponudu koju ste odabrali"></textarea>
                        <input type="hidden" name="title" value="<?php echo $product['title']; ?>" />
                        <input type="hidden" name="price" value="<?php echo $product['price']; ?>" />
                        <input type="submit" name="add_to_cart" class="btn rounded-pill font-weight-bold dodaj hover mt-2" value="Naruči" />                        
                    </div>
                </form>
            </div>
           <?php
